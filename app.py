...
vision_model.fc = torch.nn.Linear(vision_model.fc.in_features, 1) 
vision_model.eval()
...
# 2. ENHANCED PDF PARSER KEYWORDS
KEYWORDS = {
    "radius": r"radius|size|diameter",
    "texture": r"texture|surface|consistency",
    "perimeter": r"perimeter|boundary",
    "area": r"area|region",
    "smoothness": r"smoothness|evenness",
    "compactness": r"compactness|density",
    "concavity": r"concavity|indentation",
    "points": r"points|spots",
    "symmetry": r"symmetry|balance",
    "dimension": r"dimension|fractal"
}
...
def extract_from_pdf(file_path):
    ...
    try:
        ...
        for i, (key, pattern) in enumerate(KEYWORDS.items()):
            match = re.search(rf"({pattern})\s*[:=-]?\s*(\d+\.?\d*)", text, re.IGNORECASE)
            val = float(match.group(2)) if match else 15.0
...
def upload_reports():
    ...
    for file in files:
        ...
        if fname.endswith('.pdf'):
            ...
            with torch.no_grad():
                scan_score = torch.sigmoid(vision_model(img_t)).item()
                
    return jsonify({
        "message": "Aggregated Analysis Complete",
        "extracted_features": final_features,
        "scan_ready": True if scan_score > 0 else False,
        "vision_score": round(scan_score * 100, 2)
    })
